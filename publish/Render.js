...
export default class Render {
  ...
  render = async () => {
    ...
    // console.log("Rendering...", this.renderFrames[0]);
    if (this.renderFrames == null || this.renderFrames == [] || !this.renderFrames[0]) {
      clearInterval(this.renderLoopHandler);
      return;
    }

    const frame = this.renderFrames.shift().value; // console.log(JSON.stringify(frame))

    if (JSON.stringify(frame) != "{}") {
      // console.log(`Rendering ->`, frame );
      frame.forEach(event => {
        const value = event.value; // console.log(`Rendering event ->`, value );

        for (const key in value) {
          // console.log(`Rendering[${key}] ->`, event );
