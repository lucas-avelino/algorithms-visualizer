...
self.addEventListener("message", e => {
  ...
  onReceiveMessage(JSON.parse(e.data), e.origin);
});

const onReceiveMessage = (command, messageFrom) => {
...
class SorterLogic {
  ...
  constructor(sort, array) {
    // super(div);
    // this.typeInExec = sort.label;
    this.sort = this[sort.functionName];
    this.array = array;
    this.timeInExec = 0; // this.framesPerSecond = framesPerSecond;
  ...
  bubbleSort = () => {
    ...
    this.timeInExec += performance.now(); // this.prepareRender(this.eventPool);
